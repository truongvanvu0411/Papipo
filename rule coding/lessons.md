...
## 2026-04-07 - EC2 Mac simulator verification should not depend on remote VNC clicks when a dev-only bootstrap hook can prove the flow
- Symptom: The iOS simulator on EC2 Mac kept showing `Request failed` or stayed on the login screen even after the Nest API was healthy again, and VNC-based click automation could not be trusted because Accessibility permissions were not enabled on the remote Mac.
- Root Cause: Two issues were mixed together: the EC2 Mac backend was originally being run through the wrong development path, and once the API was fixed, remote GUI automation was still too flaky to prove the login flow end-to-end through manual tapping alone.
- Fix: Verified the API directly on the EC2 Mac, switched simulator verification to a gated Flutter bootstrap path using `PAPIPO_DEV_AUTO_LOGIN_EMAIL/PAPIPO_DEV_AUTO_LOGIN_PASSWORD`, and added a regression test for `SessionController.bootstrap()` so the simulator can authenticate and fetch dashboard data without changing production behavior.
- Prevention Rule: For remote iOS simulator QA on EC2 Mac, first prove the API locally on that same Mac, then prefer deterministic dev-only bootstrap hooks over VNC click automation for end-to-end verification. Keep the hook opt-in behind `dart-define` flags and cover it with tests.
- Example: `apps/mobile/lib/main.dart` now passes optional dev auto-login credentials into `SessionController`, and `apps/mobile/test/session_controller_test.dart` verifies that bootstrap can log in and load the home data when those defines are supplied.
